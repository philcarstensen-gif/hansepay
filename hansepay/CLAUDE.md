...
The headline face used in production. **All `<h1>` and `<h2>` on every page** use Georgia. Do not override this.
...
- Display font is **Georgia** (system font — no Google Fonts import needed for it). Only load Inter and Libre Baskerville from Google Fonts.
